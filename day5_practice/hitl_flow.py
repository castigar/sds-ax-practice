...
# 체크포인트는 sqlite 파일에 영구 저장됩니다. thread_id를 고정하면 다음 실행이
# 지난 대화를 이어받아 모델이 "이미 신청했다"며 도구를 안 부르고, interrupt가 없어
# state.tasks가 비어 IndexError가 납니다. 실행마다 새 스레드를 쓰면 매번 1차 실행 조건이 됩니다.
...
def no_tool_scenario(title: str):
    """tool call 없이 진행"""
    print(f"\n=== {title} ===")
    with SqliteSaver.from_conn_string("checkpoints.sqlite") as checkpointer:
        graph = builder.compile(checkpointer=checkpointer)
        config = {"configurable": {"thread_id": f"trip_{title}_{RUN_ID}"}}
